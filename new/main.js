...
(function() {
    ...
    const QUICK_COPY = {
        en: { quickWiki:'Quick Wiki', quickWikiExample:'Try "release date of PlayStation 5"', quickWikiHint:'Ask a question for a sourced, deterministic answer', indexLoading:'Loading the local web index…', indexPages:'{count} pages in the local index', openData:'Open data', noGenerativeAi:'No generative AI', fromUrl:'from URL', derivedFromUrl:'Text taken from the address of this page, because the page itself was not fetched' },
        es: { quickWiki:'Wiki rápida', quickWikiExample:'Prueba "fecha de lanzamiento de PlayStation 5"', quickWikiHint:'Haz una pregunta para obtener una respuesta determinista y con fuentes', indexLoading:'Cargando el índice web local…', indexPages:'{count} páginas en el índice local', openData:'Datos abiertos', noGenerativeAi:'Sin IA generativa', fromUrl:'de la URL', derivedFromUrl:'Texto tomado de la dirección de esta página, porque la página no se descargó' },
        fr: { quickWiki:'Wiki rapide', quickWikiExample:'Essayez « date de sortie de PlayStation 5 »', quickWikiHint:'Posez une question pour une réponse sourcée et déterministe', indexLoading:'Chargement de l’index web local…', indexPages:'{count} pages dans l’index local', openData:'Données ouvertes', noGenerativeAi:'Pas d’IA générative', fromUrl:'de l\'URL', derivedFromUrl:'Texte pris dans l\'adresse de cette page, la page n\'ayant pas été téléchargée' },
        de: { quickWiki:'Schnellwiki', quickWikiExample:'„Veröffentlichungsdatum der PlayStation 5“ ausprobieren', quickWikiHint:'Stelle eine Frage für eine determinierte Antwort mit Quellen', indexLoading:'Lokaler Web-Index wird geladen…', indexPages:'{count} Seiten im lokalen Index', openData:'Offene Daten', noGenerativeAi:'Keine generative KI', fromUrl:'aus der URL', derivedFromUrl:'Text aus der Adresse dieser Seite, da die Seite nicht geladen wurde' },
    ...
    async function performSearch(inputId) {
        ...
        // Quick Wiki is deliberately not awaited here. It needs a network round
        // trip to Wikipedia, and the web results do not, so the results paint
        // first and the summary fills in above them when it arrives.
        if (typeof window.gatewayQuickWiki === 'function') {
            const request = requestId;
            window.gatewayQuickWiki(q).then(answer => {
                if (request !== searchRequestId) return;
                quickWikiResult = answer;
                if (answer) window._gwRenderQuickWiki();
            }).catch(() => { });
        }

        try {
            const searchedResults = await window.gatewayCrawl(q);
            ...
            // Everything that needs the network lands after the first paint.
            // Each merge re-ranks rather than appending, so a late result still
            // ends up in the position its score earns it.
            const mergeLater = (promise) => {
                if (typeof promise !== 'function') return;
                const request = requestId;
                promise(q).then(extra => {
                    if (request !== searchRequestId || !extra || !extra.length) return;
                    const merged = typeof window.gatewayRank === 'function'
                        ? window.gatewayRank(allResults.concat(extra))
                        : _dedupeByUrl(allResults.concat(extra));
                    if (merged.length <= allResults.length) return;
                    allResults = _boostXpdevs(merged, q);
                    window._gwRender();
                }).catch(() => { });
            };
            if (typeof window.gatewayTailResults === 'function') {
                mergeLater(window.gatewayTailResults);
            }
            // Wikipedia, the official sites it links to, and the references it
            // cites. Started as soon as the term is known, merged in on arrival.
            mergeLater(window.gatewayWikiResults);
            return true;
        } catch(_) {
            if (requestId !== searchRequestId) return false;
    ...
    /**
     * Favicon for a result. The enriched favicon url is used when the page was
     * actually fetched, otherwise the site root is asked for /favicon.ico, and
     * if neither loads the letter avatar underneath is what shows. A broken
     * image request costs nothing visually because the letter is always there.
     */
    function _faviconHtml(r) {
        const domain = r.domain || _displayDomain(r.url) || '';
        const letter = (domain || r.title || '?').replace(/^www\./, '').charAt(0).toUpperCase();
        let src = '';
        if (r.favicon && /^https?:\/\//i.test(r.favicon)) src = r.favicon;
        // Ask the site root over the same scheme the crawler saw, otherwise
        // sites that never got a certificate return nothing for a https
        // favicon request and every result falls back to the letter.
        else if (domain) src = (r.url && r.url.charCodeAt(4) !== 115 ? 'http://' : 'https://') + domain + '/favicon.ico';
        const img = src
            ? `<img src="${_escapeHtml(src)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.remove()">`
            : '';
        return `<span class="result-favicon" aria-hidden="true"><span class="result-favicon-letter">${_escapeHtml(letter)}</span>${img}</span>`;
    }
    ...
    function renderItem(r) {
        ...
        // Only mark the results whose text came from the url rather than from
        // the page or the site. Fetched text needs no label.
        const derived = r.provenance === 2
            ? `<span class="result-derived" title="${_escapeHtml(_t('derivedFromUrl'))}">${_escapeHtml(_t('fromUrl'))}</span>`
            : '';
        return `
            <div class="result-item">
                <div class="result-item-url">
                    ${_faviconHtml(r)}
                    <span class="result-item-host">${urlDisplay}</span>
    ...
    /** Merge result lists by url, keeping the higher scoring copy. */
    function _dedupeByUrl(list) {
        const byUrl = new Map();
        for (const r of list || []) {
            if (!r || !r.url) continue;
            const key = String(r.url).replace(/#.*$/, '').replace(/\/$/, '').toLowerCase();
            const current = byUrl.get(key);
            if (!current || (r.score || 0) > (current.score || 0)) byUrl.set(key, r);
        }
    ...
    /**
     * Build the Quick Wiki box. Kept separate from the results render so the
     * summary can arrive after the web results and fill in on its own.
     */
    function _quickWikiHtml(filtered) {
        ...
        const image = quick.image
            ? `<img class="quick-wiki-image" src="${_escapeHtml(quick.image)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.remove()">`
            : '';
        return `
    ...
    /** Repaint only the Quick Wiki box, leaving the result list untouched. */
    window._gwRenderQuickWiki = function() {
        const answerArea = $('answerArea');
        if (answerArea) answerArea.innerHTML = _quickWikiHtml(allResults);
    };
    ...
    window._gwRender = function() {
        ...
        const answerArea = $('answerArea');
        if (answerArea) answerArea.innerHTML = _quickWikiHtml(filtered);
    ...
    if ($('indexStats')) {
        // The sharded index is the real one, and its size only arrives once the
        // worker has loaded the manifest. Reading only the bundled JSON count
        // made the status line say 20,000 pages while a 20.2 million document
        // index was loaded and answering queries.
        const showCount = count => {
            const stats = $('indexStats');
            if (stats) stats.textContent = count
                ? _t('indexPages', { count: count.toLocaleString(_lang) })
                : _t('indexLoading');
        };
        if (typeof window.gatewayIndexInfo === 'function') {
            const info = window.gatewayIndexInfo();
            if (info && info.docs) {
                localIndexCount = info.docs;
                showCount(info.docs);
            } else {
                // The worker has not reported in yet. Listen for it rather than
                // showing the fallback count, which is much smaller.
                window.addEventListener('gatewayIndexReady', ev => {
                    const docs = ev.detail && ev.detail.docs;
                    if (docs) { localIndexCount = docs; showCount(docs); }
                });
                if (typeof window.gatewayIndexSize === 'function') {
                    window.gatewayIndexSize().then(count => {
                        // Only fall back to this if the worker never reported.
                        if (!localIndexCount) showCount(count || 0);
                    }).catch(() => {});
                } else {
                    showCount(0);
                }
            }
        } else if (typeof window.gatewayIndexSize === 'function') {
            window.gatewayIndexSize().then(count => { localIndexCount = count || 0; showCount(count || 0); })
                .catch(() => showCount(0));
        } else {
            showCount(0);
        }
