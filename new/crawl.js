(function() {
    ...
    // ---------------------------------------------------------- sharded index
    //
    // The bundled JSON index is a fallback and the store for uploaded indexes.
    // The real index is sharded binary files read by a worker, so a query never
    // blocks the main thread and never loads the whole index.
    let _worker = null;
    let _workerReady = false;
    let _workerInfo = null;
    let _workerError = null;
    let _workerQuery = 0;
    const _workerWaiters = new Map();

    function _initShardedIndex() {
        ...
        _worker.onmessage = function (event) {
            ...
            if (msg.type === 'ready') {
                _workerReady = true;
                _workerInfo = msg;
                // Tell the page how big the real index is. The status line used to
                // read the bundled JSON count, which is the fallback, so it
                // reported 20,000 pages while this 20.2 million document index
                // was loaded and serving the queries.
                try {
                    window.dispatchEvent(new CustomEvent('gatewayIndexReady', { detail: msg }));
                } catch (_) {}
            } else if (msg.type === 'core' || msg.type === 'tail') {
                const waiter = _workerWaiters.get(msg.queryId);
                if (waiter) waiter(msg);
            } else if (msg.type === 'error') {
                _workerError = msg.message;
            }
        };
        _worker.onerror = function () { _workerError = 'worker failed'; };
        _worker.postMessage({ cmd: 'init', base: 'index/' });
    }

    // Called here, not any earlier. _initShardedIndex assigns to the _worker*
    // variables declared above, and those are `let`: running it before that
    // declaration is evaluated is a temporal dead zone ReferenceError. Function
    // declarations hoist, which is exactly why this only breaks at load and not
    // at parse time.
    _initShardedIndex();

    /** Map one index hit onto the result shape the results page already uses. */
    function _mapShardedHit(hit) {
        const doc = hit.doc || {};
        const url = doc.u || (doc.domain ? (doc.insecure ? 'http://' : 'https://') + doc.domain + (doc.path || '/') : '');
        if (!url) return null;
        const domain = _normaliseDomain(doc.domain || _domainFromUrl(url));
        return {
            title: doc.title || domain,
            url,
            description: doc.desc || '',
            fullSnippet: doc.desc || '',
            extract: doc.desc || '',
            thumbnail: doc.image || null,
            favicon: doc.favicon || '',
            source: 'gateway-sharded',
            sourceLabel: 'Gateway Index',
            resultType: 'web',
            score: hit.score,
            domain,
            suggestion: null,
            // 0 = this page was fetched, 1 = the site's own home page,
            // 2 = derived from the url because the page was not fetched.
            provenance: doc.prov === undefined ? 2 : doc.prov,
            crawled: doc.ts || 0,
            lang: doc.lang || ''
        };
    }
    ...
    /**
     * Query the sharded index. onCore fires with results that need no network,
     * onTail fires later with the results that required shard fetches.
     *
     * Returns a cancel function, so a new keystroke can drop an older request
     * instead of letting its results arrive and repaint stale content. The
     * promise for "both rounds finished" hangs off cancel.done for callers that
     * want to know when the work actually ended.
     */
    function gatewaySearchStream(term, onCore, onTail) {
        if (!_worker || !_workerReady) return null;
        const query = String(term || '').trim();
        if (!query) return null;
        const queryId = ++_workerQuery;
        const limit = 40;
        const settled = new Promise(function (resolve) {
            let coreDone = false;
            let tailDone = false;
            const finish = function () {
                if (coreDone && tailDone) {
                    _workerWaiters.delete(queryId);
                    resolve();
                }
            };
            _workerWaiters.set(queryId, function (msg) {
                ...
                if (msg.type === 'core') {
                    coreDone = true;
                    if (typeof onCore === 'function') onCore(mapped, msg);
                } else {
                    tailDone = true;
                    if (typeof onTail === 'function') onTail(mapped, msg);
                }
                finish();
            });
            _worker.postMessage({ cmd: 'query', q: query, queryId: queryId, limit: limit });
        });
        let cancelled = false;
    ...
    /** Index metadata for the status line: how many documents are searchable. */
    window.gatewayIndexInfo = function () {
        return _workerInfo || null;
    };

    window.gatewayIndexSource = function () {
        if (_workerReady) return 'sharded';
        if (_indexOverridden) return 'upload';
        return 'bundled';
    };
    ...
    window.gatewaySetIndex = async function(data) {
        ...
        // A user supplied index wins over the bundled one, so stop querying
        // the sharded index for this session.
    ...
    /**
     * Sharded index search. The core round resolves with no network at all, so
     * the page can paint, and the tail round resolves later with the results
     * that needed shard fetches. The tail promise is kept per query so
     * gatewayTailResults can hand it to the renderer.
     */
    const _tailPromises = new Map();

    function _searchSharded(term, limit) {
        const query = String(term || '').trim();
        if (!query || !_worker || !_workerReady) return null;
        const queryId = ++_workerQuery;
        let resolveCore;
        let resolveTail;
        const core = new Promise(function (r) { resolveCore = r; });
        const tail = new Promise(function (r) { resolveTail = r; });
        _tailPromises.set(query, tail);
        _workerWaiters.set(queryId, function (msg) {
            if (msg.queryId !== queryId) return;
            const mapped = (msg.results || []).map(_mapShardedHit).filter(Boolean);
            if (msg.type === 'core') resolveCore(mapped);
            else {
                _workerWaiters.delete(queryId);
                resolveTail(mapped);
            }
        });
        _worker.postMessage({ cmd: 'query', q: query, queryId: queryId, limit: limit || 40 });
        return core;
    }

    /**
     * The tail results for a query, or null when the sharded index is not in
     * use. Resolves with an empty list when the tail round produced nothing
     * new, so the caller can simply await it after the first paint.
     */
    window.gatewayTailResults = function (term) {
        const tail = _tailPromises.get(String(term || '').trim());
        return tail || Promise.resolve(null);
    };

    async function searchIndex(term) {
        const sharded = _searchSharded(term, MAX_SHARDED_RESULTS);
    ...
    async function quickWiki(query) {
        ...
        return _once(key, async () => {
            ...
            const result = {
                title: page.title,
                url: page.url,
                // The lead image comes from the same API response as the
                // extract, so the summary gains a picture without a second
                // request. The UI drops it if it fails to load.
    ...
    /**
     * The Wikipedia search for a term, shared by the Quick Wiki box and the
     * result list so one query never becomes two requests to the same api.
     */
    const _wikiPromises = new Map();
    function _wikiSearch(subject) {
        const key = _wikiLanguage() + ':' + _normalize(subject);
        let promise = _wikiPromises.get(key);
        if (!promise) {
            promise = _searchWikipedia(subject, 12)
                .then(pages => ({ pages, bestPage: _pickWikiPage(pages, subject) }))
                .catch(() => ({ pages: [], bestPage: null }));
            _wikiPromises.set(key, promise);
            if (_wikiPromises.size > 40) {
                _wikiPromises.delete(_wikiPromises.keys().next().value);
            }
        }
        return promise;
    }
    ...
    async function gatewayCrawl(term) {
        ...
        try {
            // Only the index, which is local. Wikipedia runs alongside and is
            // merged in by gatewayWikiResults once it lands, so the first paint
            // is not waiting on the network.
            const indexedGroups = await Promise.all(searchTerms.map(searchIndex));
            const results = indexedGroups.flat();

            const ranked = _dedupeAndRank(results);
            if (ranked.length) _store(key, ranked);
            if (_workerReady) {
                Promise.all(searchTerms.map(t => _tailPromises.get(t)))
                    .then(groups => {
                        const tail = groups.flat().filter(Boolean);
                        if (!tail.length) return;
                        _store(key, _dedupeAndRank(ranked.concat(tail)));
                    })
                    .catch(() => {});
            }
            // Kick Wikipedia off now so it is already in flight by the time the
            // results paint, then hand it back to the caller separately.
            _wikiSearch(parsed.subject);
            return ranked;
        } catch(_) {
    ...
    /**
     * Wikipedia-derived results for a term, ranked the same way as everything
     * else and handed to the caller for merging. Split out of gatewayCrawl so
     * the api round trip does not sit in front of the web results.
     */
    async function gatewayWikiResults(term) {
        if (!term) return [];
        const key = 'full:' + _indexRevision + ':' + _wikiLanguage() + ':' + _normalize(term);
        const parsed = parseQuickWikiQuery(term);
        const { pages, bestPage } = await _wikiSearch(parsed.subject);
        ...
        try {
            ...
            // Fold these into the cached list as well, so a repeat visit gets
            // one ranked set instead of rebuilding the merge.
            const ranked = _dedupeAndRank(results);
            if (ranked.length) {
                const existing = _cached(key);
                if (existing && existing.length) {
                    _store(key, _dedupeAndRank(existing.concat(ranked)));
                } else {
                    _store(key, ranked);
                }
            }
    ...
    window.gatewayWikiResults = gatewayWikiResults;
    window.gatewaySearchStream = gatewaySearchStream;
    window.gatewayRank = _dedupeAndRank;
